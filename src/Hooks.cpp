...
namespace MagicSneakAttacks
{
	struct detail
	{
		...
		{
			if (a_attacker->IsPlayerRef() && a_attacker->IsSneaking() && a_attacker != a_target && !a_target->IsDead()) {
				if (a_target->RequestDetectionLevel(a_attacker) <= 0) {
					if (Settings::GetSingleton()->GetSpellValid(a_spell)) {
	...
	struct SendHitEvent
	{
		static void thunk(RE::BSTEventSource<RE::TESHitEvent>& a_source, RE::TESHitEvent& a_event)
		{
			...
			if (aggressor && target && source && projectile) {
				...
				if (aggressorActor && targetActor && spell && spell->IsHostile()) {
